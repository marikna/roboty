...
class Puppy:
    ...
    def go_to_sleep(self):
        ...
        if self.touch_sensor.pressed():  #  and Button.CENTER in self.ev3.buttons.pressed()  # zmiana
            self.count_changed_timer.reset()
    ...
    def act_angry(self):
        ...
        self.stand_up()
        wait(1500)
        self.ev3.speaker.play_file(SoundFile.DOG_BARK_2) # zmiana 
        self.pet_count -= 1
    ...
    def update_behavior(self):
        ...
        elif (self.pet_count > self.pet_target and self.feed_count < self.feed_target) or (self.feed_count == 0 and self.pet_count == 0):
    ...
    def monitor_counts(self):
        ...
        if self.pet_count_timer.time() > 15000:
            ...
            print('pet_count:', self.pet_count, 'pet_target:', self.pet_target)
        if self.feed_count_timer.time() > 20000:  # zmiana
            self.feed_count_timer.reset()
            self.feed_count = max(0, self.feed_count - 1)
            print('feed_count:', self.feed_count, 'feed_target:', self.feed_target)
        if self.count_changed_timer.time() > 40000:  # zmiana
            # If nothing has happened for 40 seconds, go to sleep
            self.count_changed_timer.reset()
